...
/**
 *    \file        htdocs/core/class/fields.class.php
 *    \ingroup    core
...
class Fields
{
	...
	public static $type2label = array(
		...
		'boolean' => 'Boolean',
		'price' => 'FieldPrice',
		'pricecy' => 'FieldPriceWithCurrency',
		'phone' => 'FieldPhone',
		'mail' => 'FieldMail',
		'url' => 'FieldUrl',
		'ip' => 'FieldIP',
		...
		'radio' => 'FieldRadio',
		'select' => 'FieldSelect',
		'sellist' => 'FieldSelectList',
		'checkbox' => 'FieldCheckBox',
		'chkbxlst' => 'FieldCheckBoxFromList',
		'link' => 'FieldLink',
		'point' => 'FieldPointGeo',
		'multipts' => 'FieldMultiPointGeo',
		'linestrg' => 'FieldLinestringGeo',
		'polygon' => 'FieldPolygonGeo',
		'separate' => 'FieldSeparator',
		'stars' => 'FieldStars',
	);
	/**
	...
	public $db;
	/**
	 * @var array<string,array{label:array<string,string>,type:array<string,string>,size:array<string,string>,default:array<string,string>,computed:array<string,string>,unique:array<string,int>,required:array<string,int>,param:array<string,mixed>,perms:array<string,mixed>,list:array<string,int|string>,pos:array<string,int>,totalizable:array<string,int>,help:array<string,string>,printable:array<string,int>,enabled:array<string,int>,langfile:array<string,string>,css:array<string,string>,csslist:array<string,string>,cssview:array<string,string>,hidden:array<string,int>,mandatoryfieldsofotherentities:array<string,string>,loaded?:int,count:int}> New array to store fields definition  Note: count set as present to avoid static analysis notices
	 */
	...
	/**
	 * Return array with all possible types and labels of fields
	 *
	 * @return string[]
	 */
	 ...
	 * @param string $entity Entity of fields (for multicompany modules)
	 * @param string $langfile Language file
	 * @param string $enabled Condition to have the field enabled or not
	 * @param int<0,1> $totalizable Is a measure. Must show a total on lists
	 * @param int<0,1> $printable Is field displayed on PDF
	 ...
	 * @param string $computed Computed value
	 * @param string $entity Entity of fields
	 * @param string $langfile Language file
	 * @param string $enabled Condition to have the field enabled or not
	 * @param int<0,1> $totalizable Is field totalizable on list
	 * @param int<0,1> $printable Is field displayed on PDF
	...
	public function update(
		...
		if (isset($attrname) && $attrname != '' && preg_match("/^\w[a-zA-Z0-9-_]*$/", $attrname)) {
			...
			if (is_object($hookmanager)) {
				$hookmanager->initHooks(array('fieldsdao'));
				$parameters = array(
	...
	private function update_label(
		...
		dol_syslog(
			get_class(
				$this
			) . "::update_label " . $attrname . ", " . $label . ", " . $type . ", " . $size . ", " . $elementtype . ", " . $unique . ", " . $required . ", " . $pos . ", " . $alwayseditable . ", " . $perms . ", " . $list . ", " . $default . ", " . $computed . ", " . $entity . ", " . $langfile . ", " . $enabled . ", " . $totalizable . ", " . $printable
		);
		...
		if (isset($attrname) && $attrname != '' && preg_match("/^\w[a-zA-Z0-9-_]*$/", $attrname)) {
			...
			if ($entity === '' || $entity != '0') {
				...
				$sql_del = "DELETE FROM " . $this->db->prefix() . "fields";
			...
			$sql = "INSERT INTO " . $this->db->prefix() . "fields(";
	...
	/**
	 *    Load the array of fields definition $this->attributes
	 *
	 ...
	 * @param bool $forceload Force load of extra fields whatever is status of cache.
	...
	public function fetch_name_optionals_label($elementtype, $forceload = false, $attrname = '')
	{
		...
		// We should not have several time this request. If we have, there is some optimization to do by calling a simple $fields->fetch_optionals() in top of code and not into subcode
		...
		$sql .= " FROM " . $this->db->prefix() . "fields";
		//$sql.= " WHERE entity IN (0,".$conf->entity.")";    // Filter is done later
		if ($elementtype && $elementtype != 'all') {
			$sql .= " WHERE elementtype = '" . $this->db->escape(
					$elementtype
		...
		if ($resql) {
			...
			if ($this->db->num_rows($resql)) {
				while ($tab = $this->db->fetch_object($resql)) {
					...
					$this->attributes[$tab->elementtype]['param'][$tab->name] = ($tab->param ? jsonOrUnserialize(
						$tab->param
					) : '');
					$this->attributes[$tab->elementtype]['pos'][$tab->name] = $tab->pos;
					$this->attributes[$tab->elementtype]['alwayseditable'][$tab->name] = $tab->alwayseditable;
					$this->attributes[$tab->elementtype]['perms'][$tab->name] = ((is_null($tab->perms) || strlen(
							$tab->perms
						) == 0) ? 1 : $tab->perms);
	 ...
	 * @param string $fieldsobjectkey The key to use to store retrieved data (commonly $object->table_element)
	 * @param int $mode 1=Used for search filters
	 * @return string
	 */
	public function showInputField(
		$key,
		$value,
		$moreparam = '',
		$keysuffix = '',
		...
		if (!preg_match(
			'/options_$/',
			$keyprefix
		)) {    // Because we work on fields, we add 'options_' to prefix if not already added
			$keyprefix .= 'options_';
		}

		if (empty($fieldsobjectkey)) {
			dol_syslog(get_class($this) . '::showInputField fieldsobjectkey required', LOG_ERR);
			return 'BadValueForParamExtraFieldsObjectKey';
		}

		$label = $this->attributes[$fieldsobjectkey]['label'][$key];
		$type = $this->attributes[$fieldsobjectkey]['type'][$key];
		$size = $this->attributes[$fieldsobjectkey]['size'][$key];
		$default = $this->attributes[$fieldsobjectkey]['default'][$key];
		$computed = $this->attributes[$fieldsobjectkey]['computed'][$key];
		$unique = $this->attributes[$fieldsobjectkey]['unique'][$key];
		$required = $this->attributes[$fieldsobjectkey]['required'][$key];
		$param = $this->attributes[$fieldsobjectkey]['param'][$key];
		$perms = (int) dol_eval($this->attributes[$fieldsobjectkey]['perms'][$key], 1, 1, '2');
		$langfile = $this->attributes[$fieldsobjectkey]['langfile'][$key];
		$list = (string) dol_eval($this->attributes[$fieldsobjectkey]['list'][$key], 1, 1, '2');
		$totalizable = $this->attributes[$fieldsobjectkey]['totalizable'][$key];
		$help = $this->attributes[$fieldsobjectkey]['help'][$key];
		$hidden = (empty($list) ? 1 : 0); // If empty, we are sure it is hidden, otherwise we show. If it depends on mode (view/create/edit form or list, this must be filtered by caller)

		//var_dump('key='.$key.' '.$value.' '.$moreparam.' '.$keysuffix.' '.$keyprefix.' '.$objectid.' '.$fieldsobjectkey.' '.$mode);
		...
		if (empty($morecss)) {
			...
			if (!empty($this->attributes[$fieldsobjectkey]['css'][$key])) {
				$morecss = $this->attributes[$fieldsobjectkey]['css'][$key];
			}
		}
		...
		if (in_array($type, array('date'))) {
			...
			if ($mode == 1) {
				...
				$out .= $form->selectDate(
					$prefill['start'],
					$keyprefix . $key . $keysuffix . '_start',
					0,
					0,
					1,
					'',
				...
				$out = $form->selectDate(
					...
					'',
					1,
					(($keyprefix != 'search_' && $keyprefix != 'search_options_') ? 1 : 0),
					0,
					1
				);
			}
			...
			$tmp = explode(',', $size);
			$newsize = $tmp[0];
			$out = '<input type="text" class="flat ' . $morecss . ' maxwidthonsmartphone" name="' . $keyprefix . $key . $keysuffix . '" id="' . $keyprefix . $key . $keysuffix . '" maxlength="' . $newsize . '" value="' . dol_escape_htmltag(
					$value
				) . '"' . ($moreparam ? $moreparam : '') . '>';
		} elseif (preg_match('/varchar/', $type)) {
			$out = '<input type="text" class="flat ' . $morecss . ' maxwidthonsmartphone" name="' . $keyprefix . $key . $keysuffix . '" id="' . $keyprefix . $key . $keysuffix . '" maxlength="' . $size . '" value="' . dol_escape_htmltag(
					$value
				) . '"' . ($moreparam ? $moreparam : '') . '>';
		} elseif (in_array($type, array('mail', 'ip', 'phone', 'url'))) {
			$out = '<input type="text" class="flat ' . $morecss . ' maxwidthonsmartphone" name="' . $keyprefix . $key . $keysuffix . '" id="' . $keyprefix . $key . $keysuffix . '" value="' . dol_escape_htmltag(
					$value
				) . '" ' . ($moreparam ? $moreparam : '') . '>';
		} elseif ($type == 'icon') {
			...
			$out .= ' name="' . $keyprefix . $key . $keysuffix . '" id="' . $keyprefix . $key . $keysuffix . '" value="' . dol_escape_htmltag(
					$value
				) . '" ' . ($moreparam ? $moreparam : '') . '>';
			if (getDolGlobalInt('MAIN_ADD_ICONPICKER_JS')) {
				$out .= '<script>';
				$options = "{ title: '<b>" . $langs->trans(
						"IconFieldSelector"
				...
				$options .= "search: '<input type=\"search\" class\"form-control iconpicker-search\" placeholder=\"" . $langs->trans(
						"TypeToFilter"
					) . "\" />',";
			...
			if (!preg_match(
				...
				$doleditor = new DolEditor(
					...
					false,
					false,
					isModEnabled('fckeditor') && getDolGlobalInt('FCKEDITOR_ENABLE_SOCIETE'),
					ROWS_5,
					'90%'
				);
				$out = (string) $doleditor->Create(1);
			...
			$out = '<input type="text" class="flat ' . $morecss . ' maxwidthonsmartphone right" name="' . $keyprefix . $key . $keysuffix . '" id="' . $keyprefix . $key . $keysuffix . '" value="' . $value . '" ' . ($moreparam ? $moreparam : '') . ' placeholder="' . $langs->getCurrencySymbol(
					$conf->currency
				) . '">';
		} elseif ($type == 'pricecy') {
			...
			if ($mode) {
				...
				$out .= $form->multiselectarray(
					...
					'',
					'',
					!empty($conf->use_javascript_ajax) && !getDolGlobalString('MAIN_EXTRAFIELDS_DISABLE_SELECT2')
				);
			} else {
			...
			if (is_array($param['options'])) {
				...
				// 4 : where clause filter on column or table field, syntax field='value' or extra.field=value. Or use USF on the second line.
				...
				if (!empty($InfoFieldList[4])) {
					...
					$parenthesisopen = 0;
					while (substr($InfoFieldList[4], $pos, 1) !== '' && ($parenthesisopen || $pos == 0 || substr(
								$InfoFieldList[4],
								$pos,
								1
							) != ':')) {
					...
					// Fix better compatibility with some old field syntax filter "(field=123)"
					$reg = array();
				...
				if (!$filter_categorie) {
					...
					if (!empty($InfoFieldList[4])) {
						...
						// We have to join on field table
						$errstr = '';
						if (strpos($InfoFieldList[4], 'extra.') !== false) {
							$sql .= ' as main, ' . $this->db->sanitize(
					...
					/*
					 ...
					 $sqlwhere .= " AND invalid_usf_filter_of_field";
					 }
					 }
					 */
					...
					if ($resql) {
						...
						print 'Error in request ' . $sql . ' ' . $this->db->lasterror(
							) . '. Check setup of extra parameters.<br>';
					}
				} else {
					require_once DOL_DOCUMENT_ROOT . '/categories/class/categorie.class.php';
					$data = $form->select_all_categories(
						Categorie::$MAP_ID_TO_CODE[$InfoFieldList[5]],
						'',
						'parent',
						64,
						$InfoFieldList[6],
			...
			$out = $form->multiselectarray(
				$keyprefix . $key . $keysuffix,
				(empty($param['options']) ? null : $param['options']),
				$value_arr,
				0,
				0,
				'',
			...
			foreach ($param['options'] as $keyopt => $val) {
				...
				$out .= '/><label for="' . $keyprefix . $key . $keysuffix . '_' . $keyopt . '">' . $langs->trans(
						$val
					) . '</label><br>';
			}
							...
							while (!empty($matchCondition[0])) {
								...
								if (!empty($matchCondition[1]) && !empty($matchCondition[3]) && $matchCondition[1] != $matchCondition[3]) {
									...
									if (!empty($matchCondition[1])) {
										...
										$InfoFieldList[4] = str_replace(
											$matchCondition[0],
											$boolCond . $matchCondition[3],
											$InfoFieldList[4]
										);
			...
			$param_list = array_keys(
				$param['options']
			); // $param_list[0] = 'ObjectName:classPath' but can also be 'ObjectName:classPath:1:(status:=:1)'
			/* Removed.
			...
			$element = $fieldsobjectkey;        // $fieldsobjectkey comes from $object->table_element but we need $object->element
			...
			$out = '<input type="hidden" class="flat ' . $morecss . '" name="' . $keyprefix . $key . $keysuffix . '" id="' . $keyprefix . $key . $keysuffix . '" value="' . dol_escape_htmltag(
					$value
				) . '"' . ($moreparam ? $moreparam : '') . '>';
			$out .= '<div class="star-selection" id="' . $keyprefix . $key . $keysuffix . '_selection">';
			$i = 1;
			while ($i <= $size) {
				$out .= '<span class="star" data-value="' . $i . '">' . img_picto(
						'',
	 ...
	 * @param string $fieldsobjectkey Required (for example $object->table_element).
	 * @param Translate|null $outputlangs Output
	 * @param object $object The parent object of field to show
	 * @return    string                                    Formatted value
	 */
	public function showOutputField(
		$key,
		$value,
		$moreparam = '',
		$fieldsobjectkey = '',
		...
		if (empty($fieldsobjectkey)) {
			dol_syslog(get_class($this) . '::showOutputField fieldsobjectkey required', LOG_ERR);
			return 'BadValueForParamExtraFieldsObjectKey';
		}

		$label = $this->attributes[$fieldsobjectkey]['label'][$key];
		$type = $this->attributes[$fieldsobjectkey]['type'][$key];
		$size = $this->attributes[$fieldsobjectkey]['size'][$key];            // Can be '255', '24,8'...
		$default = $this->attributes[$fieldsobjectkey]['default'][$key];
		$computed = $this->attributes[$fieldsobjectkey]['computed'][$key];
		$unique = $this->attributes[$fieldsobjectkey]['unique'][$key];
		$required = $this->attributes[$fieldsobjectkey]['required'][$key];
		...
		$list = (string) dol_eval($this->attributes[$fieldsobjectkey]['list'][$key], 1, 1, '2');
		$help = $this->attributes[$fieldsobjectkey]['help'][$key];
		$cssview = $this->attributes[$fieldsobjectkey]['cssview'][$key];
		...
		if ($type == 'date') {
			$showsize = 10;
			if ($value !== '') {
				$value = dol_print_date(
					$value,
					'day'
				);    // For date without hour, date is always GMT for storage and output
			}
				...
				if (!$filter_categorie) {
					...
					if ($obj->rowid) {
						...
						if ($result > 0) {
							...
							); // $ways[0] = "ccc2 >> ccc2a >> ccc2a1" with html formatted text
							foreach ($ways as $way) {
								$toprint[] = '<li class="select2-search-choice-dolibarr noborderoncategories"' . ($c->color ? ' style="background: #' . $c->color . ';"' : ' style="background: #bbb"') . '>' . img_object(
										'',
										'category'
									) . ' ' . $way . '</li>';
							}
					...
					$value = '<div class="select2-container-multi-dolibarr" style="width: 90%;"><ul class="select2-choices-dolibarr">' . implode(
							' ',
							$toprint
						) . '</ul></div>';
				}
					...
					while ($obj = $this->db->fetch_object($resql)) {
						...
						if (is_array($value_arr) && in_array($obj->rowid, $value_arr)) {
							if (is_array($fields_label) && count($fields_label) > 1) {
								...
								if ($translabel != $obj->{$InfoFieldList[1]}) {
									$toprint[] = '<li class="select2-search-choice-dolibarr noborderoncategories" style="background: #bbb">' . dol_trunc(
											$translabel,
											18
										) . '</li>';
								} else {
			...
			if ($value) {        // If we have -1 here, pb is into insert, not into output (fix insert instead of changing code here to compensate)
				...
				if (!empty($classpath)) {
					...
					dol_syslog('Error bad setup of field', LOG_WARNING);
			...
			$value = '<div class="' . ($cssview ? $cssview : 'shortmessagecut') . '">' . dol_htmlentitiesbr(
					$value
				) . '</div>';
		} elseif ($type == 'html') {
			...
			$value = '<input type="hidden" class="flat" name="' . $key . '" id="' . $key . $objectid . '" value="' . dol_escape_htmltag(
					$value
				) . '"' . ($moreparam ? $moreparam : '') . '>';
			$value .= '<div class="star-selection" id="' . $key . $objectid . '_selection">';
			$i = 1;
			while ($i <= $size) {
				$value .= '<span class="star" data-value="' . $i . '">' . img_picto(
						'',
						'fontawesome_star_fas'
					) . '</span>';
				$i++;
			...
			$value .= '<script>
				$(document).ready(function() {
						...
						container.find(".star").off("click").on("click", function() {
							...
							$.ajax({
								url: "' . DOL_URL_ROOT . '/core/ajax/editfield.php",
								method: "POST",
								data: {
									objectType: \'' . dol_escape_js($fieldsobjectkey) . '\',
	...
	/**
	 * Return the CSS to use for this field into list
	 *
	 * @param string $key Key of attribute
	 * @param string $fieldsobjectkey If defined, use the new method to get fields data
	 * @return    string                            Formatted value
	 */
	public function getAlignFlag($key, $fieldsobjectkey = '')
	{
		$type = 'varchar';
		if (!empty($fieldsobjectkey)) {
			$type = $this->attributes[$fieldsobjectkey]['type'][$key];
		...
		if (!empty($this->attributes[$fieldsobjectkey]['csslist'][$key])) {
			$cssstring .= ($cssstring ? ' ' : '') . $this->attributes[$fieldsobjectkey]['csslist'][$key];
		} else {
	 ...
	 * @param string $display_type "card" for form display, "line" for document line display (fields on propal line, order line, etc...)
	...
	public function showSeparator($key, $object, $colspan = 2, $display_type = 'card', $mode = '')
	{
		...
		$field_param = $this->attributes[$object->table_element]['param'][$key];
		$field_param_list = array();
		if (!empty($field_param) && is_array($field_param)) {
			$field_param_list = array_keys($field_param['options']);
		}

		// Set $field_collapse_display_value (do we have to collapse/expand the group after the separator)
		$field_collapse_display_value = -1;
		$expand_display = false;
		if (is_array($field_param_list) && count($field_param_list) > 0) {
			$field_collapse_display_value = intval($field_param_list[0]);
			$expand_display = ((isset($_COOKIE['DOLUSER_COLLAPSE_' . $object->table_element . '_fields_' . $key]) || GETPOSTINT(
					'ignorecollapsesetup'
				)) ? (!empty($_COOKIE['DOLUSER_COLLAPSE_' . $object->table_element . '_fields_' . $key])) : !($field_collapse_display_value == 2));
		}
		...
		$out = '<' . $tagtype . ' id="trfieldseparator' . $key . (!empty($object->id) ? '_' . $object->id : '') . '" class="trfieldseparator trfieldseparator' . $key . (!empty($object->id) ? '_' . $object->id : '') . '">';
		$out .= '<' . $tagtype_dyn . ' ' . (!empty($colspan) ? 'colspan="' . $colspan . '"' : '') . '>';
		// Some js code will be injected here to manage the collapsing of fields
		// Output the picto
		$out .= '<span class="' . ($field_collapse_display_value ? 'cursorpointer ' : '') . ($field_collapse_display_value == 0 ? 'fas fa-square opacitymedium' : 'far fa-' . (($expand_display ? 'minus' : 'plus') . '-square')) . '"></span>';
		$out .= '&nbsp;';
		$out .= '<strong>';
		...
		//$fields_collapse_num = $this->attributes[$object->table_element]['pos'][$key].(!empty($object->id)?'_'.$object->id:'');

		if ($field_collapse_display_value == 1 || $field_collapse_display_value == 2) {
			...
			if (!empty($conf->use_javascript_ajax)) {
				$out .= '<!-- Add js script to manage the collapse/uncollapse of fields separators ' . $key . ' -->' . "\n";
				$out .= '<script nonce="' . getNonce() . '" type="text/javascript">' . "\n";
				$out .= 'jQuery(document).ready(function(){' . "\n";
				if (empty($disabledcookiewrite)) {
					if (!$expand_display) {
						$out .= '   console.log("Inject js for the collapsing of field ' . $key . ' - hide");' . "\n";
						$out .= '   jQuery(".trfields_collapse' . $collapse_group . '").hide();' . "\n";
					} else {
						$out .= '   console.log("Inject js for collapsing of field ' . $key . ' - keep visible and set cookie");' . "\n";
						$out .= '   document.cookie = "DOLUSER_COLLAPSE_' . $object->table_element . '_fields_' . $key . '=1; path=' . $_SERVER["PHP_SELF"] . '"' . "\n";
					}
				}
				$out .= '   jQuery("#trfieldseparator' . $key . (!empty($object->id) ? '_' . $object->id : '') . '").click(function(){' . "\n";
				$out .= '       console.log("We click on collapse/uncollapse to hide/show .trfields_collapse' . $collapse_group . '");' . "\n";
				$out .= '       jQuery(".trfields_collapse' . $collapse_group . '").toggle(100, function(){' . "\n";
				$out .= '           if (jQuery(".trfields_collapse' . $collapse_group . '").is(":hidden")) {' . "\n";
				$out .= '               jQuery("#trfieldseparator' . $key . (!empty($object->id) ? '_' . $object->id : '') . ' ' . $tagtype_dyn . ' span").addClass("fa-plus-square").removeClass("fa-minus-square");' . "\n";
				$out .= '               document.cookie = "DOLUSER_COLLAPSE_' . $object->table_element . '_fields_' . $key . '=0; path=' . $_SERVER["PHP_SELF"] . '"' . "\n";
				$out .= '           } else {' . "\n";
				$out .= '               jQuery("#trfieldseparator' . $key . (!empty($object->id) ? '_' . $object->id : '') . ' ' . $tagtype_dyn . ' span").addClass("fa-minus-square").removeClass("fa-plus-square");' . "\n";
				$out .= '               document.cookie = "DOLUSER_COLLAPSE_' . $object->table_element . '_fields_' . $key . '=1; path=' . $_SERVER["PHP_SELF"] . '"' . "\n";
				$out .= '           }' . "\n";
				$out .= '       });' . "\n";
				$out .= '   });' . "\n";
	...
	/**
	 * Fill array_options property of object by fields value (using for data sent by forms)
	 *
	 * @param null $extralabels Deprecated (old $array of fields, now set this to null)
	 * @param CommonObject $object Object
	 * @param string $onlykey Only some keys are filled:
	 *                        'string' => When we make update of only one field ($action = 'update_extras'), calling page can set this to avoid to have other fields being reset.
	 *                        '@GETPOSTISSET' => When we make update of several fields ($action = 'update'), calling page can set this to avoid to have fields not into POST being reset.
	...
	public function setOptionalsFromPost($extralabels, &$object, $onlykey = '', $todefaultifmissing = 0)
	{
		...
		if (isset($this->attributes[$object->table_element]['label']) && is_array(
				$this->attributes[$object->table_element]['label']
			)) {
		...
		if (is_array($extralabels)) {
			...
			foreach ($extralabels as $key => $value) {
				...
				if (!empty($onlykey) && $onlykey == '@GETPOSTISSET' && !GETPOSTISSET('options_' . $key) && (!in_array(
						$this->attributes[$object->table_element]['type'][$key],
						array('boolean', 'checkbox', 'chkbxlst', 'point', 'multipts', 'linestrg', 'polygon', 'duration')
					))) {
				...
				if (isset($this->attributes[$object->table_element]['enabled'][$key])) {    // 'enabled' is often a condition on module enabled or not
					$enabled = (int) dol_eval(
						(string) $this->attributes[$object->table_element]['enabled'][$key],
						1,
						1,
						'2'
					);
				...
				if (empty($enabled)
					|| (
						...
						&& in_array(
							$this->attributes[$object->table_element]['type'][$key],
							array('boolean', 'checkbox', 'chkbxlst')
						)
						&& in_array(abs($enabled), array(2, 5))
						&& !GETPOSTISSET(
							'options_' . $key
						) // Update hidden checkboxes and multiselect only if they are provided
					)
				) {
					continue;
				}
				...
				if ($this->attributes[$object->table_element]['required'][$key]) {    // Value is required
					// Check if functionally empty without using GETPOST (depending on the type of field, a
				...
				if (in_array($key_type, array('date'))) {
					...
					$value_key = dol_mktime(
						...
						0,
						0,
						GETPOSTINT("options_" . $key . "month"),
						GETPOSTINT("options_" . $key . "day"),
						GETPOSTINT("options_" . $key . "year")
					);
				} elseif (in_array($key_type, array('datetime'))) {
					// Clean parameters
					$value_key = dol_mktime(
						GETPOSTINT("options_" . $key . "hour"),
						GETPOSTINT("options_" . $key . "min"),
						GETPOSTINT("options_" . $key . "sec"),
						GETPOSTINT("options_" . $key . "month"),
						GETPOSTINT("options_" . $key . "day"),
						GETPOSTINT("options_" . $key . "year"),
						'tzuserrel'
					...
					$value_key = price2num(GETPOST("options_" . $key, 'alpha')) . ':' . GETPOST(
							"options_" . $key . "currency_id",
							'alpha'
						);
					...
					if (getDolGlobalString('MAIN_SECURITY_ALLOW_UNSECURED_REF_LABELS')) {
						...
						$label_security_check = !getDolGlobalString(
				...
				if (!empty($error_field_required[$key]) && $todefaultifmissing) {
					...
					if (!empty($this->attributes[$object->table_element]['default']) && !is_null(
							$this->attributes[$object->table_element]['default'][$key]
						)) {
	 ...
	 * @param string $type Type of field 'sellist', 'link', 'select', ...
	...
	/**
	 * return array_options array of data of fields value of object sent by a search form
	 *
	 * @param array<string,mixed>|string $fieldsobjectkey array of fields (old usage) or value of object->table_element (new usage)
	 * @param string $keysuffix Suffix string to add into name and id of field (can be used to avoid duplicate names)
	 * @param string $keyprefix Prefix string to add into name and id of field (can be used to avoid duplicate names)
	 * @return array<string,mixed>|int<0,0>                                array_options set or 0 if no value
	 */
	public function getOptionalsFromPost($fieldsobjectkey, $keysuffix = '', $keyprefix = '')
	{
		global $_POST;

		if (is_string($fieldsobjectkey) && !empty($this->attributes[$fieldsobjectkey]['label']) && is_array(
				$this->attributes[$fieldsobjectkey]['label']
			)) {
			$extralabels = $this->attributes[$fieldsobjectkey]['label'];
		} else {
			$extralabels = $fieldsobjectkey;
		}
		...
		if (is_array($extralabels)) {
			...
			foreach ($extralabels as $key => $value) {
				$key_type = '';
				if (is_string($fieldsobjectkey)) {
					$key_type = $this->attributes[$fieldsobjectkey]['type'][$key];
				}
				...
				if (in_array($key_type, array('date'))) {
					...
					if (GETPOST($dateparamname_start . 'year') || GETPOST($dateparamname_end . 'year')) {
						...
						if (GETPOST($dateparamname_start . 'year')) {
							$value_key['start'] = dol_mktime(
								0,
								0,
								0,
								GETPOSTINT($dateparamname_start . 'month'),
								GETPOSTINT($dateparamname_start . 'day'),
								GETPOSTINT($dateparamname_start . 'year')
							);
						}
						if (GETPOST($dateparamname_end . 'year')) {
							$value_key['end'] = dol_mktime(
								23,
								59,
								59,
								GETPOSTINT($dateparamname_end . 'month'),
								GETPOSTINT($dateparamname_end . 'day'),
								GETPOSTINT($dateparamname_end . 'year')
							);
						}
						...
						$value_key = dol_mktime(
							...
							0,
							0,
							GETPOSTINT($keyprefix . "options_" . $key . $keysuffix . "month"),
							GETPOSTINT($keyprefix . "options_" . $key . $keysuffix . "day"),
							GETPOSTINT($keyprefix . "options_" . $key . $keysuffix . "year")
						);
					} else {
					...
					if (GETPOST($dateparamname_start . 'year') || GETPOST($dateparamname_end . 'year')) {
						...
						$dateparamname_start_hour = GETPOSTINT($dateparamname_start . 'hour') != '-1' ? GETPOSTINT(
							$dateparamname_start . 'hour'
						) : '00';
						$dateparamname_start_min = GETPOSTINT($dateparamname_start . 'min') != '-1' ? GETPOSTINT(
							$dateparamname_start . 'min'
						) : '00';
						$dateparamname_start_sec = GETPOSTINT($dateparamname_start . 'sec') != '-1' ? GETPOSTINT(
							$dateparamname_start . 'sec'
						) : '00';
						$dateparamname_end_hour = GETPOSTINT($dateparamname_end . 'hour') != '-1' ? GETPOSTINT(
							$dateparamname_end . 'hour'
						) : '23';
						$dateparamname_end_min = GETPOSTINT($dateparamname_end . 'min') != '-1' ? GETPOSTINT(
							$dateparamname_end . 'min'
						) : '59';
						$dateparamname_end_sec = GETPOSTINT($dateparamname_end . 'sec') != '-1' ? GETPOSTINT(
							$dateparamname_end . 'sec'
						) : '59';
						if ($key_type == 'datetimegmt') {
							$value_key = array(
								'start' => dol_mktime(
									$dateparamname_start_hour,
									$dateparamname_start_min,
									$dateparamname_start_sec,
									GETPOSTINT($dateparamname_start . 'month'),
									GETPOSTINT($dateparamname_start . 'day'),
									GETPOSTINT($dateparamname_start . 'year'),
									'gmt'
								...
								'end' => dol_mktime(
									$dateparamname_end_hour,
									$dateparamname_end_min,
									$dateparamname_end_sec,
									GETPOSTINT($dateparamname_end . 'month'),
									GETPOSTINT($dateparamname_end . 'day'),
									GETPOSTINT($dateparamname_end . 'year'),
									'gmt'
						...
						if ($key_type == 'datetimegmt') {
							$value_key = dol_mktime(
								GETPOSTINT($keyprefix . "options_" . $key . $keysuffix . "hour"),
								GETPOSTINT($keyprefix . "options_" . $key . $keysuffix . "min"),
								GETPOSTINT($keyprefix . "options_" . $key . $keysuffix . "sec"),
								GETPOSTINT($keyprefix . "options_" . $key . $keysuffix . "month"),
								GETPOSTINT($keyprefix . "options_" . $key . $keysuffix . "day"),
								GETPOSTINT($keyprefix . "options_" . $key . $keysuffix . "year"),
								'gmt'
					...
					$value_key = dol_htmlcleanlastbr(
						GETPOST($keyprefix . "options_" . $key . $keysuffix, 'restricthtml')
					);
				} else {
					if (!GETPOSTISSET($keyprefix . "options_" . $key . $keysuffix)) {
						continue; // Value was not provided, we should not set it.
